...
export default function VereinsAssistent() {
  ...
  return (
    <div className="bg-white dark:bg-viktoria-dark-light rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 h-full flex flex-col">
      ...
      {/* Chat Messages Area */}
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 min-h-0 max-h-full">
          ...
          >
            <div className={`flex items-start space-x-2 max-w-[85%] ${message.sender === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
              <div className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center ${
                ...
                {message.sender === 'user' ? (
                  <IconUser size={16} className="text-white dark:text-gray-900" />
                ) : (
                  <IconRobot size={16} className="text-viktoria-blue dark:text-viktoria-yellow" />
                )}
              </div>
      ...
      <div className="border-t border-gray-200 dark:border-viktoria-dark-lighter p-3 flex-shrink-0">
        <div className="flex space-x-2">
          <input
            ...
            className="flex-1 px-3 py-2 bg-gray-50 dark:bg-viktoria-dark-lighter border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-viktoria-blue dark:focus:ring-viktoria-yellow focus:border-transparent"
          />
          <button
            onClick={handleSend}
